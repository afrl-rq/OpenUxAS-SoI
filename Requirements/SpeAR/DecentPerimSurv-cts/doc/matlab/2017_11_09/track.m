...
for n=1:N,
   uav(n).x   = pos(n);
   uav(n).v   = vel(n);
   uav(n).nl  = 0; % n-1;      % # UAVs to left
   uav(n).nr  = 0; % N-n;      % # UAVs to right
   uav(n).pl  = pos(n);   % estimate of perimeter to left
   uav(n).pr  = 1-pos(n); % estimate of perimeter to right
...
while t < T,
   ...
   % process events
   for i=1:length(r),
      if abs(r(i) - rmin) < 1e-12, 
          if i <= N, % UAV with ID ind(i) reached goal
             uav = UpdateUav(uav, i, 'goal');
          end;
          if i >= N+2 && i <= 2*N, % two UAV rend;ezvous
             right = i - N; % right UAV in rend;ezvous
...
function updated = TurnAround(uav, i, eventtype)
...
disp(['  >> ', num2str(i), ':  TL ', num2str(uav(i).tl), ', TR ', num2str(uav(i).tr), ' Pos: ', num2str(uav(i).x)]);
