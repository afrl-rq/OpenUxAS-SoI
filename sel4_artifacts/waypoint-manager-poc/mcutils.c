...
void PrintMC(FILE * f, mc_t * mc_ptr) {
  ...
  for(i = 0 ;i < mc_ptr->full.waypointssize ; i++) {
    double * lat = (double*)&(mc_ptr->full.waypoints[i].latitude);
    double * lng = (double*)&(mc_ptr->full.waypoints[i].longitude);
    float * alt = (float*)&(mc_ptr->full.waypoints[i].altitude);
    float * spd = (float*)&(mc_ptr->full.waypoints[i].speed);
