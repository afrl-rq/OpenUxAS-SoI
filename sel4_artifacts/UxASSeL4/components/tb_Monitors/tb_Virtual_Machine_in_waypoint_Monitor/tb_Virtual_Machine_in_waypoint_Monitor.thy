 ...
 * two modifications:
 * 1) The line in "src/tb_Virtual_Machine_in_waypoint_Monitor.c" defining TB_VERIFY
 *    must be uncommented.
 * 2) The AUTOCORRES_HOME text below must be replaced by the path to the 
 *    AutoCorres package for Isabelle.
 *)
theory tb_Virtual_Machine_in_waypoint_Monitor
...
install_C_file "src/tb_Virtual_Machine_in_waypoint_Monitor.c"
autocorres[ts_rules = nondet, heap_abs_syntax] "src/tb_Virtual_Machine_in_waypoint_Monitor.c"

context tb_Virtual_Machine_in_waypoint_Monitor begin
...
apply (unfold mon_dequeue'_def)
  apply wp
  apply (auto simp: fun_upd_def is_queue_def the_queue_def tb_Virtual_Machine_in_waypoint_Monitor.update_MissionSoftware__waypoint_impl_def
