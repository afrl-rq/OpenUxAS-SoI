 ...
 * two modifications:
 * 1) The line in "src/tb_Waypoint_Manager_mission_write_Monitor.c" defining TB_VERIFY
 *    must be uncommented.
...
apply (unfold mon_dequeue'_def)
  apply wp
  apply (metis (mono_tags, hide_lams) fun_upd_def is_queue_def the_queue_def tb_Waypoint_Manager_mission_write_Monitor.heap_w32_update_def
