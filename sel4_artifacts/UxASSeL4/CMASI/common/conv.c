...
int lmcp_unpack_int16_t (uint8_t** buf, size_t *size_remain, int16_t* out) {
    int16_t j; int res = lmcp_unpack_uint16_t(buf, size_remain, (uint16_t*)&j);
...
int lmcp_unpack_int32_t (uint8_t** buf, size_t *size_remain, int32_t* out) {
    int32_t j; int res = lmcp_unpack_uint32_t(buf, size_remain, (uint32_t*)&j);
    if (buf != NULL && *buf != NULL && res == 0) {
    *out= *(int32_t*)&j;
    return 0;
    }
...
int lmcp_unpack_int64_t (uint8_t** buf, size_t *size_remain, int64_t* out) {
    int64_t j; int res = lmcp_unpack_uint64_t(buf,size_remain,(uint64_t*)&j);
    if (buf != NULL && *buf != NULL && res == 0) {
    *out = *(int64_t*)&j;
    return 0;
    }
