 ...
 * two modifications:
 * 1) The line in "src/tb_Waypoint_Manager_in_mission_Monitor.c" defining TB_VERIFY
 *    must be uncommented.
 * 2) The AUTOCORRES_HOME text below must be replaced by the path to the 
 *    AutoCorres package for Isabelle.
 *)
theory tb_Waypoint_Manager_in_mission_Monitor
...
install_C_file "src/tb_Waypoint_Manager_in_mission_Monitor.c"
autocorres[ts_rules = nondet, heap_abs_syntax] "src/tb_Waypoint_Manager_in_mission_Monitor.c"

context tb_Waypoint_Manager_in_mission_Monitor begin
