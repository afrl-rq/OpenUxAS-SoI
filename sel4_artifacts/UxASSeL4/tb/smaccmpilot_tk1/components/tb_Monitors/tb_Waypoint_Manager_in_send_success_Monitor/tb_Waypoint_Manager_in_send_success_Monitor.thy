 ...
 * two modifications:
 * 1) The line in "src/tb_Waypoint_Manager_in_send_success_Monitor.c" defining TB_VERIFY
 *    must be uncommented.
 * 2) The AUTOCORRES_HOME text below must be replaced by the path to the 
 *    AutoCorres package for Isabelle.
 *)
theory tb_Waypoint_Manager_in_send_success_Monitor
...
install_C_file "src/tb_Waypoint_Manager_in_send_success_Monitor.c"
autocorres[ts_rules = nondet, heap_abs_syntax] "src/tb_Waypoint_Manager_in_send_success_Monitor.c"

context tb_Waypoint_Manager_in_send_success_Monitor begin
...
apply (unfold mon_dequeue'_def)
  apply wp
  apply (metis (mono_tags, hide_lams) fun_upd_def is_queue_def the_queue_def tb_Waypoint_Manager_in_send_success_Monitor.heap_w8_update_def
