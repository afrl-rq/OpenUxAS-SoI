 ...
 * two modifications:
 * 1) The line in "src/tb_Waypoint_Manager_in_waypoint_Monitor.c" defining TB_VERIFY
 *    must be uncommented.
 * 2) The AUTOCORRES_HOME text below must be replaced by the path to the 
 *    AutoCorres package for Isabelle.
 *)
theory tb_Waypoint_Manager_in_waypoint_Monitor
...
install_C_file "src/tb_Waypoint_Manager_in_waypoint_Monitor.c"
autocorres[ts_rules = nondet, heap_abs_syntax] "src/tb_Waypoint_Manager_in_waypoint_Monitor.c"

context tb_Waypoint_Manager_in_waypoint_Monitor begin
...
definition the_queue :: "lifted_globals \<Rightarrow> MissionSoftware__waypoint_impl_C queue" where
...
lemma enqueue_not_full:
  "(\<And>s c. P (s\<lparr>contents_'' := c\<rparr>) = P s) \<Longrightarrow>
   ...
   \<lbrace> \<lambda>s. is_queue s \<and>
         ...
         is_valid_MissionSoftware__waypoint_impl_C s x \<and>
         P s \<rbrace>
   mon_enqueue' x
   \<lbrace> \<lambda>r s. r = 1 \<and>
           the_queue s = enqueue q (heap_MissionSoftware__waypoint_impl_C s x) \<and>
           is_queue s \<and>
           P s \<rbrace>!"
apply (unfold mon_enqueue'_def)
apply wp
...
lemma dequeue_not_empty:
  "(\<And>s f. P (s\<lparr>front_'' := f\<rparr>) = P s) \<Longrightarrow>
   ...
   \<lbrace> \<lambda>r s. r > 0 \<and>
           dequeue q = (heap_MissionSoftware__waypoint_impl_C s x, the_queue s) \<and>
           is_queue s \<and>
           P s \<rbrace>!"
apply (unfold mon_dequeue'_def)
  apply wp
  apply (auto simp: fun_upd_def is_queue_def the_queue_def tb_Waypoint_Manager_in_waypoint_Monitor.update_MissionSoftware__waypoint_impl_def
