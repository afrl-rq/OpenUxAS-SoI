...
# from Tool_Installation_and_Setup_Instructions.docx, 2017-06-30:
...
if [ "$(uname)" == "Darwin" ]; then
    ...
    echo "First -- update Ubuntu Linux packages if needed!"
...
echo "Install Oracle Java run-time (required for LmcpGen)"
if [ "$(uname)" == "Darwin" ]; then
#if [ "$($(uname -s) | cut -c 1-6)" == "Darwin" ]; then
...
# same for both platforms ("safe" because exited out earlier if not MacOSX or Linux already):
...
# same for both platforms ("safe" because exited out earlier if not MacOSX or Linux already):
echo "Uninstall the AGREE and Resolute Plugins inside OSATE2 (prerequisite for installing updated plugins)"
echo "* Select 'Help'->'Installation Details'"
echo "* In the 'Installed Software' tab, select 'Agree' and 'Resolute' from the list of installed software"
echo "  (for multi-select: left-click on 'Agree', hold down Ctrl key, left-click on 'Resolute', let go of Ctrl key)"
echo "* Click 'Uninstall...'"
echo "* In the 'Uninstall' dialogue box, click 'Finish'"
...
# same for both platforms ("safe" because exited out earlier if not MacOSX or Linux already):
echo "Install the (updated) AGREE and Resolute Plugins, and the Z3 Plugin (used by AGREE), inside OSATE2"
...
echo "* Wait for the list to load, then open out 'SMACCM' and check the 'Agree', 'Resolute', and 'Z3 Plugin' boxes"
