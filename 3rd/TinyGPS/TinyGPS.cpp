...
bool TinyGPS::term_complete()
{
  if (_is_checksum_term)
  {
    BYTE_t checksum = 16 * from_hex(_term[0]) + from_hex(_term[1]);
    if (checksum == _parity)
    {
      if (_gps_data_good)
      {
...
float TinyGPS::distance_between (float lat1, float long1, float lat2, float long2) 
{
  ...
  delta = pow(delta,0.5f); 
  delta += pow((clat2 * sdlong),0.5f); 
  delta = sqrt(delta); 
...
void TinyGPS::crack_datetime(int *year, BYTE_t *month, BYTE_t *day, 
  BYTE_t *hour, BYTE_t *minute, BYTE_t *second, BYTE_t *hundredths, unsigned long *age)
{
