    ...
    namespace common
    {
        namespace utilities
        {
            ...
            enum Token {ALTERNATIVE = 10, SEQUENTIAL = 11, PARALLEL = 12, RELATIVE = 13,
                ...
                BRACKET_RIGHT = 18, PAREN_LEFT = 19, PAREN_RIGHT = 20, TASK = 21,
                CARAT = 22, UNDEFINED = 2000};
            enum Flags {};

            //Constructor
            Lex::Lex(std::string tmpAlgebraString, std::map<int, std::array<std::vector<std::vector<int> >, 2> > * m_map){
                //Init var
            ...
            void Lex :: rDec() {
                ...
                if (lex() != 0) {
                    ...
                    //If error, should not reach EOF. Send error and exit.
                    /**
            ...
            int Lex :: lex() {
                ...
                switch(charClass) {
                    case OPERAND:
                        ...
                        //If prereq flags are set (if there is timing and prereq, this runs first, then timing)
                        else {
                            if (set_prereq) {
                    ...
                    case TIME:
                        ...
                        break;
                    case UNKNOWN:
                        std::cout << "lex_branch_UNKNOWN: " << nextToken << std::endl;
                        //Ignore nested loops when processing timing
                        if (timingFlags.at(0)) {
                            if (nextToken == CARAT) {
                                std::cout << "lex_branch_timing_CARAT" << std::endl;
            ...
            void Lex :: skipToSectionEnd(bool goForward) {
                ...
                if (goForward) {
                    ...
                    //Process the string in a backwards directiongetNonBlank
                else {
                    do {
            ...
            /**
             * Skip from current pos to char, changing global pos. Choose direction.
             * @param ch //char: char to find
             * @param isBackward //bool: true to move forwards, false backwards
             */
            bool Lex :: skipToChar(char ch) {
                char tmp;

                std::cout << "skipToChar: " << ch << std::endl;
                while (((tmp = getNonBlank()) != ch) && (tmp != 0)) {}
