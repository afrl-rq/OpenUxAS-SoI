...
LmcpObjectNetworkPublishPullBridge::configure(const pugi::xml_node& bridgeXmlNode)
{
    ...
    if (!bridgeXmlNode.attribute(STRING_XML_ADDRESS_PULL).empty())
    {
        ...
        LOG_INFORM(s_typeName(), "::configure setting external pull socket address to ", m_externalPullSocketAddress, " from XML configuration");
    }
    else
    {
        LOG_INFORM(s_typeName(), "::configure failed to find external pull socket address in XML configuration; external pull socket address is ", m_externalPullSocketAddress);
    }
    ...
    if (!bridgeXmlNode.attribute(STRING_XML_ADDRESS_PUB).empty())
    {
        ...
        LOG_INFORM(s_typeName(), "::configure setting external pub socket address to ", m_externalPublishSocketAddress, " from XML configuration");
    }
    else
    {
        LOG_INFORM(s_typeName(), "::configure failed to find external pub socket in XML configuration; external pub socket is ", m_externalPublishSocketAddress);
    }
...
LmcpObjectNetworkPublishPullBridge::initialize()
{
    ...
    LOG_INFORM(s_typeName(), " external pull socket address is ", m_externalPullSocketAddress);
    ...
    LOG_INFORM(s_typeName(), " external pub socket address is ", m_externalPublishSocketAddress);
...
LmcpObjectNetworkPublishPullBridge::start()
{
    ...
    LOG_INFORM(s_typeName(), "::start pull receive processing thread [", m_externalReceiveProcessingThread->get_id(), "]");
    return (true);
};
