			...
			* */
				...
				(vehicleAssignmentState->m_isAcceptingNewAssignments))
			{
				...
				if (travelTime_ms >= 0)
				{
					...
					{
						UXAS_LOG_ERROR("ASSIGNMENT_ERROR:: required timing information for TaskOptionId[", taskId, "] not found!");
						m_staticAssignmentParameters->m_reasonsForNoAssignment << "ASSIGNMENT_ERROR:: required timing information for TaskOptionId[" << taskId << "] not found!" << std::endl;
						isError = true;
					}
