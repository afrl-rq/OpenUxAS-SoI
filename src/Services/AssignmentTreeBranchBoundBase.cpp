	...
	namespace service
	{
		...
		void c_Node_Base::ExpandNode()
		{
			...
			for (auto itObjectiveID = vectorOfNextObjectiveIDs.begin(); itObjectiveID != vectorOfNextObjectiveIDs.end(); itObjectiveID++) // ALGEBRA:: New for loop
			{
				...
				//TODO:: find if there is a prerequsite task
        ...
        //added function
        int64_t c_Node_Base::isFoundPrerequisites(int64_t prerequisiteTaskId){
            c_Node_Base * backtraceCheck = this;
            while(backtraceCheck != backtraceCheck->m_parentPointer){
                if(backtraceCheck->m_taskOptionID / 100000 == prerequisiteTaskId){
                    return backtraceCheck->m_taskOptionID;
                }
			...
			* */
				...
				(vehicleAssignmentState->m_isAcceptingNewAssignments))
			{
				...
				if (travelTime_ms >= 0)
				{
					...
					if (!m_tasksToTime->at(taskId)[0].empty() || !m_tasksToTime->at(taskId)[1].empty()) //Has at least a timing operator
					{
						std::cout << "Has a timing operator" << std::endl;
						if (!m_tasksToTime->at(taskId)[0].empty() && m_tasksToTime->at(taskId)[0].at(0).at(0) == 14) //Absolute time
						{
							std::cout << "Test" << std::endl;
							prerequisiteTime_ms = m_tasksToTime->at(taskId)[0].at(0).at(1);
						}
						else if (!m_tasksToTime->at(taskId)[1].empty() || (!m_tasksToTime->at(taskId)[0].empty() && m_tasksToTime->at(taskId)[0].at(0).at(0) == 13)) //Relative time
						{
							std::cout << "In relative time" << std::endl;
							int64_t lowestAlternativePrerequisiteCost(INT64_MAX);
							if (!m_tasksToTime->at(taskId)[1].empty()) //Has some prerequisite
							{
								std::cout << "Has prerequsite in relative" << std::endl;
								for (unsigned int i = 0; i < m_tasksToTime->at(taskId)[1].at(0).size(); i++)
								{
									int64_t prerequisiteTaskOptionId = isFoundPrerequisites(m_tasksToTime->at(taskId)[1].at(0).at(i));
									std::cout << "prerequisiteTaskId: " << prerequisiteTaskOptionId << std::endl;
									if (prerequisiteTaskOptionId > 0) //Confirm we have a prerequisite
									{
										...
										if (itTaskAssignmentState != m_taskIdVsAssignmentState.end())
										{
											...
											if (itTaskAssignmentState->second->m_taskCompletionTime_ms > prerequisiteTime_ms)
											{
												...
												else if (prerequisiteOperator == 10) // '+' operator
												{
													if (itTaskAssignmentState->second->m_taskCompletionTime_ms < lowestAlternativePrerequisiteCost) //Lowest alternative will be selected
													{
										...
										{
											UXAS_LOG_ERROR("ASSIGNMENT_ERROR:: potentially required prerequisite TaskOptionId[", taskId, "] not found");
              ...
              else if(!m_tasksToTime->at(taskId)[0].empty()) //Relative time without a prerequisite; treat as absolute
              {
                  prerequisiteTime_ms = m_tasksToTime->at(taskId)[0].at(0).at(1);
              }
					...
					{
						/*!isError can be split out very easily, you just need to have an "else" that logs an error message and
            ...
            //std::cout << prerequisiteTime_ms << std::endl;
						if ((maxVehicleTravelTime_ms < 0) || (travelTimeTotalToEnd_ms < maxVehicleTravelTime_ms))
						{
							...
							if (nodeCost < m_staticAssignmentParameters->m_minimumAssignmentCostCandidate)
							{
								...
								//BEGIN ADDED CODE
								newChild->m_taskIdVsAssignmentState[taskOptionId]->m_taskBeginTime_ms = travelTimeTotalToBegin_ms;
								//END ADDED CODE
								//////// update the vehicle //////////
