...
void LineSegment::makeTestPoints(int k){
	double delta = 1.0/((double) k);
	for (int i=0; i < k; ++i){
		double h = i * delta;
		double xt = s_A.first *(1.0 - h) + s_B.first * h;
		double yt = s_A.second *(1.0 -h) + s_B.second * h;
		coord_t pt(xt, yt);
		testPoints.push_back(pt);
		_robustness.push_back(-10000.0);
	}
}

LineSegment::LineSegment(int id, double xa, double ya, double xb, double yb, int numTestPoints): s_id(id),
	s_A(xa, ya),
	s_B(xb, yb),
	s_C(0.0, 0.0, 0.0)
{
	computeSegmentEqFromEndPoints(s_A, s_B, s_C);
	makeTestPoints(numTestPoints);
}
...
void LineSegment::registerSensorFootprint(MonitorPolygon const & mp) {
	/*--
	   0. Calculate for each test point the robustness according to mp and update the overall robustness.
	  1. Iterate through all the half-spaces of this polygon.
	         1.1. Compute intersection with the segment
	    2. Add interval
	  --*/
	static_assert(std::numeric_limits<double>::is_iec559, "IEEE 754 required");
	int kk = testPoints.size();
	for (int i =0; i < kk ; ++i){
		auto const & testPt = testPoints[i];
		double rob;
		bool inside;
		std::tie(inside, rob) = mp.isMember(testPt.first, testPt.second);
		// Make sure that this is as we expect
		if (inside) assert(rob >= 0.0);
		else assert(rob <= 0.0);
		_robustness[i] = std::max(_robustness[i], rob);
	}
...
bool LineSegment::isSegmentCovered() const {
...
bool IntervalList::coversUnit() const {
	if (intervals.size() != 1) {
		return false;
	}
	auto const & p = *(intervals.begin());
	return (p.first <= EPS && p.second >= 1.0 - EPS);
}

void IntervalList::printAll() const {
