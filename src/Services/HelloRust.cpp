 ...
 * Author: acfoltzer
 *
 * <Service Type="HelloRust" StringToSend="Option_01" SendPeriod_ms="36" />
 * 
 */
...
HelloRust::ServiceBase::CreationRegistrar<HelloRust>
HelloRust::s_registrar(HelloRust::s_registryServiceTypeNames());

// service constructor
HelloRust::HelloRust()
: ServiceBase(HelloRust::s_typeName(), HelloRust::s_directoryName()) { };

// service destructor
HelloRust::~HelloRust() { };


bool HelloRust::configure(const pugi::xml_node& ndComponent)
{
    bool isSuccess(true);
...
bool HelloRust::terminate()
{
    ...
    {
        UXAS_LOG_WARN(s_typeName(), "::HelloRust::terminate() failed to destroy message send timer ",
                 "with timer ID ", m_sendMessageTimerId, " within ", delayTime_ms, " millisecond timeout");
    }
    return (true);
}

bool HelloRust::processReceivedLmcpMessage(std::unique_ptr<uxas::communications::data::LmcpMessage> receivedLmcpMessage)
{
