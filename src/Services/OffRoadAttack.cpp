 ...
 *
 * <Service Type="OffRoadAttack" StringToSend="Option_01" SendPeriod_ms="36" />
 * 
 */

// include header for this service
#include "OffRoadAttack.h"
...
#define CERR_FILE_LINE_MSG(MESSAGE) std::cerr << "<>OffRoadAttack:: " << __FILE__ << ":" << __LINE__ << ":" << MESSAGE << std::endl;std::cout.flush();
...
#define STRING_XML_ATTACK_FIRST_WAYPOINT_LAT "AttackFirstLat"
#define STRING_XML_ATTACK_FIRST_WAYPOINT_LON "AttackFirstLon"
#define COUT_INFO(MESSAGE) std::cout << "<>!!!OffRoadAttack:: " << MESSAGE << std::endl;std::cout.flush();
...
OffRoadAttack::ServiceBase::CreationRegistrar<OffRoadAttack>
OffRoadAttack::s_registrar(OffRoadAttack::s_registryServiceTypeNames());

// service constructor
OffRoadAttack::OffRoadAttack()
: ServiceBase(OffRoadAttack::s_typeName(), OffRoadAttack::s_directoryName()) { };

// service destructor
OffRoadAttack::~OffRoadAttack() { };


bool OffRoadAttack::configure(const pugi::xml_node& ndComponent)
{
        ...
        if (!ndComponent.attribute(STRING_XML_NUMBER_WAYPOINTS_TO_SERVE).empty())
    {
        ...
        if (!ndComponent.attribute(STRING_XML_ATTACK_FIRST_WAYPOINT_LAT).empty())
    {
        attackFirstLat = ndComponent.attribute(STRING_XML_ATTACK_FIRST_WAYPOINT_LAT).as_double();
    }
        if (!ndComponent.attribute(STRING_XML_ATTACK_FIRST_WAYPOINT_LON).empty())
    {
        attackFirstLon = ndComponent.attribute(STRING_XML_ATTACK_FIRST_WAYPOINT_LON).as_double();
    }
    ...
    //fetch the first "Type" element in configuration file, which is "OffRoadAttack"
...
OffRoadAttack::initialize()
{
    ...
    m_sendNewMissionTimerId = uxas::common::TimerManager::getInstance().createTimer(
                                                                                    std::bind(&OffRoadAttack::OnSendNewMissionTimer, this), "OffRoadAttack::OnSendNewMissionTimer");
...
bool OffRoadAttack::processReceivedLmcpMessage(std::unique_ptr<uxas::communications::data::LmcpMessage> receivedLmcpMessage)
{
...
bool OffRoadAttack::isGetNextWaypointId(const int64_t& waypointIdCurrent, int64_t & waypointIdNext)
{
...
bool OffRoadAttack::isGetCurrentSegment(const int64_t& waypointIdCurrent, std::shared_ptr<avtas::lmcp::Object>& segmentCurrent, int64_t & idMissionSegmentCurrent)
{
    bool isSucceeded(false);
...
bool OffRoadAttack::isInitializePlan(std::shared_ptr<afrl::cmasi::MissionCommand> & ptr_MissionCommand)
{
    ...
    if (m_vehicleID > 0)
    {
        ...
        if (!ptr_MissionCommand->getWaypointList().empty())
        {
            ...
            double initLat= attackFirstLat;//45.33523635499905
            double initLon= attackFirstLon;//-121.00452002670227
            ...
            for (; itWaypoint != ptr_MissionCommand->getWaypointList().end(); itWaypoint++)
            {
                ...
                auto myPoint = (*itWaypoint)->clone();
                myPoint->setLatitude(initLat);//-0.001
                myPoint->setLongitude(initLon);//-0.001
                //// altitude can not be set here
                myPoint->setAltitude(1000);
                myPoint->setClimbRate(20.0);
                
                initLat+=0.001;
                initLon-=0.001;
                ////if set speed here, it will only change the speed for the modified waypoints and the speed of u and ground speed, not air speed
                ////setring speedType and Altitude are not working here
                myPoint->setSpeedType(afrl::cmasi::SpeedType::SpeedType::Airspeed);
                myPoint->setSpeed(24);
                
                
                float speed = myPoint->getSpeed();
                ...
                {
                    missionSegment_01->getWaypointList().push_back(myPoint);
                    ...
                    {
                        missionSegment_02->getWaypointList().push_back(myPoint);
                    }
                ...
                else
                {
                    ...
                    {
                        CERR_FILE_LINE_MSG("ERROR::OffRoadAttack::isInitializePlan:: first waypoint of segment was not set!!!!")
                        isSucceeded = false;
                    }
                }
            ...
            {
                CERR_FILE_LINE_MSG("ERROR::OffRoadAttack::isInitializePlan:: no m_missionSegments constructed from the MissionCommand!!!!")
                isSucceeded = false;
            }
        }
        else
        {
            CERR_FILE_LINE_MSG("ERROR::OffRoadAttack::isInitializePlan:: no waypoints found in the MissionCommand!!!!")
            isSucceeded = false;
        }
    }
    else
    {
        CERR_FILE_LINE_MSG("ERROR::OffRoadAttack::isInitializePlan:: vehicle ID not > 0!!!!")
        isSucceeded = false;
    }
    return (isSucceeded);
};

void OffRoadAttack::setTurnType(const afrl::cmasi::TurnType::TurnType& turnType, std::shared_ptr<afrl::cmasi::MissionCommand> & ptr_MissionCommand)
{
...
void OffRoadAttack::OnSendNewMissionTimer()
{
