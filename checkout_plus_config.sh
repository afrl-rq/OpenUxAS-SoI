...
if [ "$DOWNLOAD_VS_COMPILE" == "-d" ]; then
    ...
    if [ "$(uname)" == "Darwin" ]; then
    #if [ "$($(uname -s) | cut -c 1-6)" == "Darwin" ]; then
        /Applications/Safari.app/Contents/MacOS/Safari & sleep 1 && osascript -e 'tell application "Safari" to open location "https://github.com/$REPO_SOURCE_OA/OpenAMASE/releases/download/v1.3.0/OpenAMASE.jar"'
    elif [ "$(expr substr $(uname -s) 1 5)" == "Linux" ]; then
        firefox https://github.com/$REPO_SOURCE_OA/OpenAMASE/releases/download/v1.3.0/OpenAMASE.jar &
    fi
...
if [ "$DOWNLOAD_VS_COMPILE" == "-d" ]; then
    ...
    if [ "$(uname)" == "Darwin" ]; then
    #if [ "$($(uname -s) | cut -c 1-6)" == "Darwin" ]; then
        /Applications/Safari.app/Contents/MacOS/Safari & sleep 1 && osascript -e 'tell application "Safari" to open location "https://github.com/$REPO_SOURCE_LG/LmcpGen/releases/download/v1.5.0/LmcpGen.jar"'
    elif [ "$(expr substr $(uname -s) 1 5)" == "Linux" ]; then
        firefox https://github.com/$REPO_SOURCE_LG/LmcpGen/releases/download/v1.5.0/LmcpGen.jar &
    fi
    read -rs -p " " -n 1 # reference: https://ss64.com/bash/read.html
    echo " "
elif [ "$DOWNLOAD_VS_COMPILE" == "-c" ]; then
